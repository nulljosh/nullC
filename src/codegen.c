// codegen.c — x86-64 assembly code generator for macOS (Darwin)
// Targets System V AMD64 ABI, AT&T syntax
...
// arg register names for System V AMD64
...
static void codegen_lvalue(CodeGen *cg, ASTNode *node) {
    switch (node->type) {
        case AST_IDENTIFIER: {
            ...
            emit(cg, "    leaq %d(%%rbp), %%rax", cg->locals[li].offset);
            break;
        }
...
static void codegen_expr(CodeGen *cg, ASTNode *node) {
    ...
    switch (node->type) {
        case AST_NUMBER:
            emit(cg, "    movq $%d, %%rax", node->data.number.value);
            break;

        case AST_CHAR_LIT:
            emit(cg, "    movq $%d, %%rax", (int)(unsigned char)node->data.char_lit.value);
            break;

        case AST_STRING: {
            int lbl = register_string(cg, node->data.string_lit.value);
            emit(cg, "    leaq .str%d(%%rip), %%rax", lbl);
            break;
        }
        ...
        case AST_IDENTIFIER: {
            ...
            // If it's a struct value, return the address (structs are passed by address)
                ...
                emit(cg, "    movq %d(%%rbp), %%rax", cg->locals[li].offset);
            }
            break;
        }
        ...
        case AST_BINARY_OP: {
            ...
            if (strcmp(op, "+") == 0) {
                ...
                // left - right: rdi - rax
                emit(cg, "    subq %%rax, %%rdi");
                emit(cg, "    movq %%rdi, %%rax");
            }
            else if (strcmp(op, "*") == 0) {
                emit(cg, "    imulq %%rdi, %%rax");
            }
            else if (strcmp(op, "/") == 0) {
                // left / right: rdi / rax
                // dividend in rax, divisor in rcx
                emit(cg, "    movq %%rax, %%rcx");    // right (divisor) -> rcx
                emit(cg, "    movq %%rdi, %%rax");    // left (dividend) -> rax
                emit(cg, "    cqto");                  // sign extend rax -> rdx:rax
                emit(cg, "    idivq %%rcx");           // rax = quotient
            }
            else if (strcmp(op, "%") == 0) {
                ...
                emit(cg, "    cqto");
                emit(cg, "    idivq %%rcx");
                emit(cg, "    movq %%rdx, %%rax");     // remainder
            }
            else if (strcmp(op, "==") == 0) {
                emit(cg, "    cmpq %%rax, %%rdi");
                emit(cg, "    sete %%al");
        ...
        case AST_CALL: {
            ...
            // After our prologue's pushq %rbp, rsp is 16-byte aligned if we entered
            // aligned. Our subq for locals is aligned to 16. Each pushq/popq pair
...
static void codegen_stmt(CodeGen *cg, ASTNode *node) {
    ...
    switch (node->type) {
        ...
        case AST_BREAK: {
            ...
            emit(cg, "    jmp .L%d", cg->break_labels[cg->break_depth - 1]);
            break;
        }
        ...
        case AST_VAR_DECL: {
            ...
            if (node->data.var_decl.init) {
                ...
                if (is_struct_type(type) && ptr_level == 0) {
                    ...
                    emit(cg, "    movq %%rax, %d(%%rbp)", cg->locals[li].offset);
                } else {
                    codegen_expr(cg, init);
                    emit(cg, "    movq %%rax, %d(%%rbp)", cg->locals[li].offset);
                }
...
static void codegen_function(CodeGen *cg, ASTNode *node) {
    ...
    emit(cg, "    subq $%d, %%rsp", total_stack);
        ...
        if (is_struct_type(ptype) && pptr == 0) {
            ...
            if (i < 6) {
                ...
                for (int f = 0; f < nfields; f++) {
                    emit(cg, "    movq %d(%s), %%rax", f * 8, arg_regs[i]);
                    emit(cg, "    movq %%rax, %d(%%rbp)", cg->locals[li].offset + f * 8);
                }
            }
        } else {
            // Scalar parameter
            ...
            if (i < 6) {
                emit(cg, "    movq %s, %d(%%rbp)", arg_regs[i], cg->locals[li].offset);
            }
